    ...
    public = list(
        ...
        validate_subset = function(subset, j, n) {
            ...
            if (!all(j %in% subset)) {
                ...
                stop(sprintf("Input error in subset or j vector: Column indices (j) are not present in training data (subset). Extra elements in j are %s",
                     dif))
            }
        },
        ...
        validate_phi = function(phi, n) {
            msg <- checkmate::check_numeric(phi, lower = 0,
                                            any.missing = F, len = n,
                                            finite = TRUE)
            private$.print_error(
        ...
        validate_sigma = function(sigma, P_2) {
            msg <- checkmate::check_matrix(sigma, mode = "numeric",
                                           nrows = P_2, ncols = P_2)
            private$.print_error(
        ...
        validate_x = function(Xf, Xr) {
            msg_xf <- checkmate::check_matrix(Xf, mode = "numeric",
                                              any.missing = F,
            ...
            if (!is.null(Xr)) {
                msg_xr <- checkmate::check_matrix(Xr, mode = "numeric",
                                                  any.missing = F,
